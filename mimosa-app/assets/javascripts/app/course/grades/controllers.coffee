define ['angular'], (angular) ->
	return angular.module('djangoApp.controllers').controller 'GradesController', ($scope, $routeParams, $location, Restangular, User, Course, Lesson, Assignment, AssignmentSubmission, GradedAssignmentSubmission) ->
	...
	# Create an empty list of lessons to avoid any null references
	$scope.lessons = []

	# Only attempt to load lessons for this course if there are any
	if $scope.course.lessons.length > 0
		Lesson.all($scope.course.lessons).then (lessons) ->
			$scope.lessons = lessons

	# Create an empty lis of sections and section members to avoid any null references
	$scope.sections = []
	$scope.section_members = []

	# Only attempt to load sections and members if there are any
	if $scope.course.sections.length > 0
		...
		CourseSection.all($scope.course.sections).then (sections) ->
			...
			if $scope.isInstructor
				# We need to combine all of the roster references into a single list to load
				rosterIds = []
				rosterIds.push.section.members for section in $scope.sections

				# This flattens the array to keep it as a single dimension
				rosterIds = _.flatten(rosterIds)

				# Retrieve just the roster entries that belong to the student group
				CourseRoster.students(rosterIds).then (members) ->
					# This is our list of student roster objects
					$scope.section_members = members

					# Load the actual user data for each student in our section members list
					#
					# Pluck out just the user parameter, as that is the reference to the user id
					User.all(_.pluck(members, 'user')).then (students) ->
						$scope.students = students

					# We need to combine all of the team references into a single list to load
					teamIds = []
